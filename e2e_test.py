...
def register_user(username: str) -> str:
    response: Response = requests.post(
        ...
        json={"login": username, "password": "passawubasd!!"},
    )
...
def ping_game_server(host: str, port: int) -> dict[str, Any]:
    ...
    with socket.socket(family=socket.AF_INET, type=socket.SOCK_DGRAM) as sock:
        ...
        message = b"PING"
        _ = sock.sendto(message, (host, port))
...
def queue_user(token: str, username: str, ws_timeout: float) -> bool:
    ...
    match_config = ping_game_server(
        host=cast(str, payload["host"]), port=int(cast(int, payload["port"]))
    )
...
def add_n_users(n: int):
    ...
    results = requests.get(
        "http://localhost:3000/api/results", cookies={"session": tokens[0]}
    ).json()
    ...
    for i in range(n):
        results = requests.get(
            "http://localhost:3000/api/results", cookies={"session": tokens[i]}
        ).json()
        assert isinstance(results, dict)
...
def test_crash_server_manager(kill_before_queue: bool) -> None:
    ...
    kill: Thread = threading.Thread(
        ...
        args=(
            ["multiplayer-asset-server-manager-1"],
            5,
        ),
    )
...
def test_crash_game_server() -> None:
    ...
    kill: Thread = threading.Thread(target=kill_container, args=(containers,))
...
def test_browser(page: Page) -> None:
    ...
    with zipfile.ZipFile(
        "./data/downloads/game-client.zip", mode="w", compression=zipfile.ZIP_DEFLATED
    ) as zip_file:
